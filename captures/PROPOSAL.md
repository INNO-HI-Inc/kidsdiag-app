...
| ![](./fun-1-1.png) | ![](./fun-1-2.png) | ![](./fun-1-3.png) |
|---|---|---|
| **그림 1-1.** 소속 학교·학년 자동 매칭 — 로그인 정보 기반 진단 검사지 자동 배정 | **그림 1-2.** 진단 응시 화면 — 4지선다 보기 + 정답채점기준 모달 + 정·오답 자가 표시 + 멀티미디어 재생 + 하단 sticky 제출 버튼 | **그림 1-3.** 수식·멀티미디어 문항 에디터 — Σ 수식·특수기호·이미지·음성·동영상 편집 툴바 9종 + 메타데이터 23필드 |
...
| ![](./fun-2-1.png) | ![](./fun-2-2.png) | ![](./fun-2-3.png) |
|---|---|---|
| **그림 2-1.** 학생용 종합 결과 — 종합 마스터리 큰 숫자 + 영역별 가로 막대 + 랭킹·진도 2×2 + 8각 재능 레이더 + 학습 가이드 | **그림 2-2.** 학부모 리포트 PDF 3페이지 — P1 학력 결과(전국 분포 백분위) / P2 재능 프로필(발달민감기) / P3 심화 진단 안내 | **그림 2-3.** 교원 학급 단위 통계 — 27명 학생 응시 결과·사전→사후 향상도·반 평균 마스터리 + Excel 다운로드 |
...
| ![](./fun-7-1.png) | ![](./fun-7-2.png) | ![](./fun-7-3.png) |
|---|---|---|
| **그림 7-1.** 4역할 × 12기능 권한 매트릭스 — 가능/조건부/불가 3단계 표시 + 학생 대상자별 검색 + 알림 발송 폼 (앱푸시·SMS·이메일) | **그림 7-2.** 회원관리 — 학생/교원/학부모 테이블 + 신규 등록 폼 + 승인 대기 / 활성 상태 + 수정·삭제 액션 | **그림 7-3.** 만 14세 미만 법정대리인 동의 5종 — 개인정보보호법 §22조의2 + 학력·재능·종단추적·마케팅 분리 + IRB·PIA·5년 자동 파기 안내 |

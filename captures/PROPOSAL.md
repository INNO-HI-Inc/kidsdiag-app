...
|---|---|
| **그림 1-1.** 소속 학교·학년 자동 매칭 — 로그인 정보 기반 진단 검사지 자동 배정 | **그림 1-2.** 진단 시작 전 안내 — 총 45분 / 4개 교과 블록 자동 진행 (국어·과학·수학·통합사고력) |

| ![](./fun-1-3.png) | ![](./fun-1-4.png) |
|---|---|
| **그림 1-3.** 진단 응시 화면 — 4지선다 보기 + 정답채점기준 모달 + 정·오답 자가 표시 + 멀티미디어 재생 + 하단 sticky 제출 버튼 | **그림 1-4.** 수식·멀티미디어 문항 에디터 — Σ 수식·특수기호·이미지·음성·동영상 편집 툴바 9종 + 메타데이터 23필드 |
...
|---|---|
| **그림 7-1.** 회원관리 — 학생/교원/학부모 4역할별 필터 + 계정 등록·수정·삭제·승인 액션 + 활성/승인대기 상태 관리 | **그림 7-2.** 4역할 × 12기능 권한 매트릭스 — 가능/조건부/불가 3단계 + 학생 대상자별 검색 + 알림 발송 폼 (앱푸시·SMS·이메일) |
